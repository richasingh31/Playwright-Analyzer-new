...
function extractTenantId(reportName: string): string | null {
...
// Same date source used by the Trends page's "All Reports" table
function reportEffectiveDate(rep: ParsedReport): string {
  return rep.metadata?.startTime ? new Date(rep.metadata.startTime).toISOString() : rep.uploadedAt;
}
...
function buildTenantData(reports: ParsedReport[]): {
  ...
  // Determine tenant key per report. Reports whose name carries a tenant ID are
  // grouped by that ID (latest report wins). Reports without one each get their
  // own column — keyed by report id so same-day uploads don't collapse together —
  // labeled with their date instead of the (often meaningless) filename.
  const tenantMap = new Map<string, ParsedReport>(); // tenantKey → latest report
  const tenantLabels = new Map<string, string>(); // tenantKey → display label
  reports.forEach((rep) => {
    const tenantId = extractTenantId(rep.name);
    const key = tenantId ? `tenant:${tenantId}` : `report:${rep.id}`;
    const label = tenantId
      ? `Tenant ${tenantId}`
      : new Date(reportEffectiveDate(rep)).toLocaleDateString('en-US', {
          year: 'numeric',
          month: 'short',
          day: 'numeric',
        });
    ...
    if (!existing || new Date(rep.uploadedAt) > new Date(existing.uploadedAt)) {
      tenantMap.set(key, rep);
      tenantLabels.set(key, label);
    }
  });

  const tenants = Array.from(tenantMap.keys()).sort((a, b) => {
    const idA = extractTenantId(tenantMap.get(a)!.name);
    const idB = extractTenantId(tenantMap.get(b)!.name);
    // Tenant-ID columns first (sorted numerically), then date-labeled columns (sorted chronologically)
    if (idA && idB) return parseInt(idA, 10) - parseInt(idB, 10);
    if (idA && !idB) return -1;
    if (!idA && idB) return 1;
    return new Date(reportEffectiveDate(tenantMap.get(a)!)).getTime() - new Date(reportEffectiveDate(tenantMap.get(b)!)).getTime();
  });
  ...
  tenantMap.forEach((report, tenantKey) => {
    report.suites.forEach((suite) => {
      ...
      getAllTests(suite).forEach((test) => {
        ...
        scenarioMap.get(test.fullTitle)!.tenantStatuses.set(tenantKey, {
          status: test.status,
  ...
  return { rows, tenants, tenantLabels, stats };
}
...
function TenantCell({ ts }: { ts: TenantStatus | undefined }) {
  ...
  if (ts.status === 'passed')
    return <CheckCircle2 className="h-4 w-4 text-emerald-600 mx-auto" />;
  if (ts.status === 'failed')
    return <XCircle className="h-4 w-4 text-red-600 mx-auto" />;
  if (ts.status === 'flaky')
    return <AlertCircle className="h-4 w-4 text-amber-600 mx-auto" />;
...
export function TenantComparisonPage() {
  ...
  const { rows, tenants, tenantLabels, stats } = useMemo(() => {
    if (reports.length === 0)
      return {
        rows: [],
        tenants: [],
    ...
    return (
      <div className="mx-auto max-w-7xl px-6 py-10">
        <div className="rounded-2xl border border-slate-300/60 bg-slate-200/60 p-10 text-center">
          ...
          <h2 className="text-lg font-semibold text-slate-900 mb-2">Need at least 2 tenant reports</h2>
          <p className="text-slate-600 text-sm max-w-md mx-auto mb-6">
            Upload reports whose names contain tenant identifiers (e.g. <code className="text-indigo-700">TenantID:1</code>, <code className="text-indigo-700">TenantID:4</code>) to see cross-tenant divergence analysis.
          </p>
      ...
      {grouped.length === 0 ? (
        ...
        <div className="space-y-4">
          {grouped.map(([apiName, apiRows]) => (
            <ApiGroupBlock key={apiName} apiName={apiName} rows={apiRows} tenants={tenants} tenantLabels={tenantLabels} />
          ))}
        </div>
      )}
    </div>
    ...
    <div className="rounded-2xl border border-slate-300/60 bg-slate-200/60 overflow-hidden backdrop-blur-sm">
      ...
      {open && (
        <div className="border-t border-slate-300/40">
          ...
          <div className="py-1">
            {rows.map((row) => (
              <ScenarioRow key={row.fullTitle} row={row} tenants={tenants} tenantLabels={tenantLabels} />
            ))}
          </div>
        </div>
      )}
