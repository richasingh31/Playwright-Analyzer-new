    ...
    <Card>
      ...
      <div className="space-y-1.5 max-h-80 overflow-y-auto pr-1">
        {rows.map((r) => (
          <div
            key={r.key}
            className="flex items-start justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50/60 px-3 py-2"
          >
            <div className="min-w-0">
              <p className="text-sm font-medium text-slate-800 break-words">
                {r.label}
              </p>
              <p className="text-xs text-slate-500 break-all mt-0.5" title={r.file}>
                {r.file.split(/[\\/]/).slice(-2).join('/')}
              </p>
            </div>
            <div className="shrink-0 text-xs font-medium whitespace-nowrap mt-0.5" style={{ color: accentColor }}>
              {r.right}
            </div>
          </div>
        ))}
